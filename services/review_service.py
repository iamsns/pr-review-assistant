...
from llm.chains.risk_score_chain import chain as risk_score_chain
...
class ReviewService:
    ...
    def generate_test_case(self, pr_data:PROverviewRequest):
        ...
        return "Invalid Data"
    def get_risk_score(self, pr_data:PROverviewRequest):
        overview = self.overview_service.generate_overview(pr_data)
        if overview:
            print("Overview:- ", overview)
            return risk_score_chain.invoke({"diff":overview})
